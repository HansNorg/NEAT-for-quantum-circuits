...
def li_h_instance(distance = None):
    if distance == None:
...
def get_solution():
    ...
    for ind, i in enumerate(x):
        instance = li_h_instance(i)
        y[ind] = exact_diagonalisation(LithiumHydride.hamiltonian(instance)) + instance["correction"]
    return x, y
...
class AllLithiumHydride(LithiumHydride):
    def energy(self, circuit, parameters, no_optimization=False, instance=None, no_solution=False) -> float:
