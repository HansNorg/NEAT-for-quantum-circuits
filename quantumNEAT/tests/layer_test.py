...
sys.path.append(sys.path[0]+"\\..\quantumneat")
import unittest
import quantumneat.layer as l
import quantumneat.gate as g
import quantumneat.logger as log
...
class TestLayer(unittest.TestCase):
    ...
    def test_layer(self):
        ...
        reference_circuit2.barrier()
        self.logger.debug(f"Reference circuit: \n{reference_circuit.draw(fold=-1)}")
        self.logger.debug(f"Reference circuit2: \n{reference_circuit2.draw(fold=-1)}")
        self.logger.debug(f"Test circuit: \n{test_circuit.draw(fold=-1)}")
