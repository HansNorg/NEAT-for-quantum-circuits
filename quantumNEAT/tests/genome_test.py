...
from ..quantumneat import genome as gen
from ..quantumneat import gate as g
from ..quantumneat import layer as l
from ..quantumneat import helper as h
from ..quantumneat import logger as log
...
class TestGenome(unittest.TestCase):
    def setUp(self):
        ...
        self.gate1 = g.GateGene(0, g.GateTypes.ROT, 0)
        self.gate2 = g.GateGene(1, g.GateTypes.ROT, 1)
        self.gate3 = g.GateGene(2, g.GateTypes.CNOT, 0)
        self.gate4 = g.GateGene(3, g.GateTypes.CNOT, 1)
        self.gate5 = g.GateGene(4, g.GateTypes.ROT, 0)
        self.gate6 = g.GateGene(5, g.GateTypes.CNOT, 1)
    ...
    def test_compute_gradient(self):
        ...
        for k in range(n_different):
            ...
            layers[0].add_gate(g.GateGene(0, g.GateTypes.ROT, 0))
            layers[0].add_gate(g.GateGene(1, g.GateTypes.ROT, 1))
            layers[0].add_gate(g.GateGene(2, g.GateTypes.CNOT, 0))
            layers[0].add_gate(g.GateGene(3, g.GateTypes.CNOT, 1))
...
def from_layers(global_layer_numer, layers):
    genome = gen.Genome(global_layer_numer)
    genome.layers = layers
    return genome

if __name__ == '__main__':
    log.QuantumNEATLogger("test", file_level=logging.DEBUG, mode="w")
    unittest.main()